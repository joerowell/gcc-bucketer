...
inline void FastHadamardLSH::insert_in_maxs(int32_t *const maxs, const int16_t val,
                                            const int32_t index)
{
  ...
  // If we're only hashing against one bucket then pack the value and its index in maxs (maxs is of
  // size 2) and return
  int i = multi_hash - 1;
  if (!i)
  {
  ...
  // As we deal with pairs, we need to decrement this here to make sure we start in the right
  // position
  i--;
...
inline void FastHadamardLSH::insert_in_maxs_epi16(int32_t *const maxs, const int i_high,
                                                  const Simd::VecType vals)
{
  ...
  if (Simd::m256_testz_si256(tmp, tmp))
    return;
...
void FastHadamardLSH::hash_templated(const int16_t *const vv, int32_t *const res)
{
  ...
  // If we have more than 2 registers to operate on then we apply the Hadamard transform on 32
  // entries at a given time However, if we've got only 2 registers to operate on then we need to
  // use different permutations and we only apply the Hadamard transform on 16 entries at a given
  // time.
  //
  // But we don't want to use a regular if! That's slow.
  // Thankfully, if constexpr is a thing - it will throw away the branch that we don't care about in
  // our template - thus it lets us write simpler code, but get the benefits we want in the end.
  if constexpr (regs_ > 2)
  {
    ...
    m256_permute_epi16<regs_>(v, prg_state, tailmask, aes_key);
...
void FastHadamardLSH::hash(const float *const v, float *const coeff, int32_t *const hashes)
{
  ...
  case 2:
  {
    FastHadamardLSH::hash_templated<2>(vv, res);
    break;
  }
  case 3:
  {
    FastHadamardLSH::hash_templated<3>(vv, res);
    break;
  }
  case 4:
  {
    FastHadamardLSH::hash_templated<4>(vv, res);
    break;
  }
  case 5:
  {
    FastHadamardLSH::hash_templated<5>(vv, res);
    break;
  }
  case 6:
  {
    FastHadamardLSH::hash_templated<6>(vv, res);
    break;
  }
  case 7:
  {
    FastHadamardLSH::hash_templated<7>(vv, res);
    break;
  }
  case 8:
  {
    FastHadamardLSH::hash_templated<8>(vv, res);
    break;
  }
  default:
...
inline bool ProductLSH::insert_in_maxs(float *const scores, int32_t *const indices,
                                       const float score, const int32_t index)
{
  ...
  for (unsigned i0 = 0; i0 < multi_hash_block; ++i0)
  {
    ...
    for (; i1 < multi_hash_block; ++i1)
    {
      ...
      for (; i2 < multi_hash_block; ++i2)
      {
        ...
        if (!insert_in_maxs(c, res, c2_, sign0 * (h2_ + 1)))
          break;
      }
...
template <> void ProductLSH::hash_templated<2>(const float *const vv, int32_t *const res)
{
  ...
  for (unsigned i0 = 0; i0 < multi_hash_block; ++i0)
  {
    ...
    for (; i1 < multi_hash_block; ++i1)
    {
      ...
      if (!insert_in_maxs(c, res, c1_, sign0 * (h1_ + 1)))
        break;
    }
...
void ProductLSH::hash(const float *const v, int32_t *const res)
{
  ...
  if (blocks == 1)
  {
    hash_templated<1>(vv, res);
  }
  else if (blocks == 2)
  {
    hash_templated<2>(vv, res);
  }
  else if (blocks == 3)
  {
    hash_templated<3>(vv, res);
  }
  else
  {
